...
def enviar_email_boas_vindas(nome, email, url, senha):
                ...
                <td style="padding:24px 28px;">
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      ...
                      <td style="padding:6px 0;color:#111827;font-weight:600;">admin</td>
                    </tr>
                    <tr>
...
"""
    ...
    try:
        with urllib.request.urlopen(req) as r:
            ...
            print(f'[EMAIL] Enviado para {email}: {resp}')
...
def criar_cliente(nome, email, plano='starter'):
    slug = re.sub(r'[^a-z0-9]', '', email.split('@')[0].lower())[:16]
    if os.path.exists(f'{CLIENTES_DIR}/{slug}'):
        slug = slug + secrets.token_hex(2)

    db_pass     = secrets.token_urlsafe(20)
    jwt_secret  = secrets.token_urlsafe(32)
    admin_senha = secrets.token_urlsafe(10)
    admin_hash  = gerar_hash(admin_senha)
    anon_jwt    = gerar_jwt(jwt_secret)
    pasta       = f'{CLIENTES_DIR}/{slug}'
    os.makedirs(pasta, exist_ok=True)

    for f in ['Dockerfile', 'nginx.conf', 'entrypoint.sh', 'lista-compras-saas.html']:
        shutil.copy(f'/home/ubuntu/orchestrator/templates/{f}', pasta)

    schema = open('/home/ubuntu/orchestrator/templates/schema.sql').read()
    schema = (schema
        .replace('{{NOME}}',       nome)
        .replace('{{PLANO}}',      plano)
...
"""
    ...
    enviar_email_boas_vindas(nome, email, url, admin_senha)

    return {
        'ok': True, 'cliente_id': slug,
        'url': url,
        'login': 'admin', 'senha': admin_senha, 'plano': plano
    }
...
def criar_manual():
    ...
    return jsonify(criar_cliente(data['nome'], data['email'], data.get('plano','starter')))
